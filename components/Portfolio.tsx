...
import { Github, Linkedin, Mail, ExternalLink, Shield, Cpu, Award, Briefcase, Code, Terminal, ChevronDown, Menu, X, FileText, Download } from 'lucide-react';
...
const Portfolio = () => {
  ...
  const projects = [
    {
      ...
      description: "Full-spectrum security platform combining ML-powered phishing detection (90%+ accuracy) with hardware-based wireless defense and cloud honeypot infrastructure.",
    ...
    },
    {
      ...
      description: "IoT-enabled agricultural solution using CNN for real-time crop disease detection, with automated farmer notifications via cloud integration.",
      tags: ["ML", "IoT"],
    ...
    },
    {
      ...
      description: "Enterprise-grade React application automating NIST SP 800-61 incident response plan generation with 92% enterprise validation rating.",
      tags: ["Security", "Web"],
    ...
    },
    {
      ...
      description: "Production-ready computer vision system for automated part inspection using transfer learning, deployed as standalone executable for factory floor operations.",
      tags: ["ML", "Industrial"],
  ...
  return (
    <>
      {/* RESUME MODAL - top-level so it renders above everything */}
      <AnimatePresence>
        {showResumeMenu && (
          <motion.div
      ...
      {/* MAIN PAGE CONTENT */}
      <div className={`min-h-screen ${isDark ? 'bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900' : 'bg-gradient-to-br from-gray-50 to-gray-100'} transition-colors duration-300`}>
        {/* Animated Background */}
        ...
        {/* Navigation */}
        <nav className={`fixed top-0 w-full z-50 backdrop-blur-md ${isDark ? 'bg-slate-900/80 border-b border-cyan-500/20' : 'bg-white/80 border-b border-gray-200'}`}>
          ...
          <AnimatePresence>
            {mobileMenuOpen && (
              <motion.div
                ...
                className={`md:hidden ${isDark ? 'bg-slate-900' : 'bg-white'} border-t ${isDark ? 'border-cyan-500/20' : 'border-gray-200'}`}
              >
                {navItems.map((item) => (
                  <button
                    key={item.id}
                ...
                <div className="max-w-5xl mx-auto text-center">
                  ...
                  >
                    <h1 className={`text-5xl md:text-7xl font-bold mb-4 ${isDark ? 'text-white' : 'text-gray-900'}`}>
                      Hi, I'm <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-purple-500">Atharva</span>
                    </h1>
                    <div className="flex items-center justify-center space-x-4 mb-6">
                      <Shield className="text-cyan-400" size={32} />
                      <span className={`text-2xl md:text-3xl ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>×</span>
                      <Cpu className="text-purple-400" size={32} />
                    </div>
                    <p className={`text-xl md:text-2xl ${isDark ? 'text-gray-300' : 'text-gray-600'} mb-8`}>
                      Security-Aware AI Engineer | Building Intelligent Defense Systems
                    </p>
                  </motion.div>
                  ...
                  <motion.p
                    ...
                    className={`text-lg ${isDark ? 'text-gray-400' : 'text-gray-600'} max-w-3xl mx-auto mb-12`}
                  >
                    Bridging cybersecurity and machine learning to create robust, intelligent systems. 
                    From ML-powered threat detection to secure IoT deployments, I engineer solutions 
                  ...
                  >
                    ...
                    <button
                      ...
                      className={`px-8 py-3 border-2 ${isDark ? 'border-cyan-400 text-cyan-400 hover:bg-cyan-400/10' : 'border-cyan-600 text-cyan-600 hover:bg-cyan-50'} rounded-lg font-semibold transition-all`}
                    >
                      Get In Touch
                    </button>
                  </motion.div>
                  ...
                  >
                   ...
                   <button onClick={() => setShowResumeMenu(true)} className="text-gray-400 hover:text-cyan-400 transition-colors">
                      <FileText size={28} />
                    </button>
                  </motion.div>
              ...
              >
                <div className="max-w-5xl mx-auto">
                  ...
                  <div className="grid md:grid-cols-2 gap-8">
                    <motion.div
                      {...fadeInUp}
                      className={`p-6 rounded-xl ${isDark ? 'bg-slate-800/50 border border-cyan-500/20' : 'bg-white border border-gray-200'} backdrop-blur`}
                    >
                      <h3 className={`text-2xl font-bold mb-4 ${isDark ? 'text-white' : 'text-gray-900'}`}>Background</h3>
                      <p className={`${isDark ? 'text-gray-300' : 'text-gray-700'} leading-relaxed`}>
                        Electronics & Telecommunication Engineering student with a unique dual expertise 
                        in cybersecurity and machine learning. Currently maintaining a 9.02 CGPA while 
                        building production-grade AI systems and security infrastructure.
                      </p>
                    </motion.div>
                    ...
                    <motion.div
                      ...
                      className={`p-6 rounded-xl ${isDark ? 'bg-slate-800/50 border border-purple-500/20' : 'bg-white border border-gray-200'} backdrop-blur`}
                    >
                      <h3 className={`text-2xl font-bold mb-4 ${isDark ? 'text-white' : 'text-gray-900'}`}>Approach</h3>
                      <p className={`${isDark ? 'text-gray-300' : 'text-gray-700'} leading-relaxed`}>
                        I build systems that don't compromise between intelligence and security. Every ML 
                        model I deploy considers threat vectors, and every security system I design 
                        leverages AI for enhanced detection and response.
                      </p>
                    </motion.div>
                  </div>
                  ...
                  >
                    ...
                    <div className="grid md:grid-cols-2 gap-4">
                      {certifications.map((cert, idx) => (
                        <div
                          key={idx}
                          className={`p-4 rounded-lg ${isDark ? 'bg-slate-800/30 border border-cyan-500/10' : 'bg-gray-50 border border-gray-200'}`}
                        >
                          <div className="flex items-start space-x-3">
                            <Award className="text-cyan-400 flex-shrink-0 mt-1" size={20} />
                            <div>
                              <h4 className={`font-semibold ${isDark ? 'text-white' : 'text-gray-900'}`}>{cert.name}</h4>
                              <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>{cert.issuer} • {cert.date}</p>
                            </div>
                          </div>
                        </div>
                      ))}
                      {achievements.map((achievement, idx) => (
                        <div
                          key={idx}
                          className={`p-4 rounded-lg ${isDark ? 'bg-slate-800/30 border border-purple-500/10' : 'bg-gray-50 border border-gray-200'}`}
                        >
                          <div className="flex items-start space-x-3">
                      ...
                      >
                        <div className="flex flex-col md:flex-row md:items-start md:justify-between mb-4">
                          <div className="flex-1">
                            ...
                            <p className={`text-sm ${isDark ? 'text-cyan-400' : 'text-cyan-600'} mb-2`}>
                              {project.subtitle}
                            </p>
                        ...
                        <div className="mb-4">
                          <h4 className={`text-sm font-semibold ${isDark ? 'text-gray-400' : 'text-gray-600'} mb-2`}>
                            Key Highlights:
                          </h4>
                          ...
                          {project.tech.map((tech, i) => (
                            <span
                              ...
                              className={`px-2 py-1 text-xs ${
                                isDark ? 'bg-slate-700/50 text-gray-300' : 'bg-gray-100 text-gray-700'
                              } rounded`}
                            >
                              {tech}
                            </span>
                ...
                <div className="max-w-6xl mx-auto">
                  ...
                  <div className="space-y-12">
                    {Object.entries(skills).map(([category, skillList], catIdx) => (
                      ...
                      >
                        <h3 className={`text-2xl font-bold mb-6 capitalize ${isDark ? 'text-white' : 'text-gray-900'}`}>
                          ...
                          {skillList.map((skill, idx) => (
                            <div
                              ...
                              className={`p-4 rounded-lg ${
                                isDark ? 'bg-slate-800/50 border border-cyan-500/20' : 'bg-white border border-gray-200'
                              }`}
                            >
                              ...
                              <div className={`w-full h-2 ${isDark ? 'bg-slate-700' : 'bg-gray-200'} rounded-full overflow-hidden`}>
                                <motion.div
                                  initial={{ width: 0 }}
                  ...
                  >
                    ...
                    <div className="flex flex-wrap gap-3">
                      {[
                        ...
                        'Docker', 'AWS', 'Linux', 'Git', 'MySQL', 'REST APIs',
                        'Bash', 'C++', 'Java', 'Electron.js', 'NumPy', 'Pandas'
                      ].map((tech, idx) => (
                        <motion.span
                          key={idx}
                          whileHover={{ scale: 1.05 }}
                ...
                <div className="max-w-5xl mx-auto">
                  ...
                  >
                    ...
                    <div className="space-y-4">
                      <h4 className={`font-semibold ${isDark ? 'text-gray-300' : 'text-gray-700'} mb-3`}>
                        Key Contributions:
                      </h4>
                    ...
                    <div className="mt-6 pt-6 border-t border-cyan-500/20">
                      <div className="flex flex-wrap gap-2">
                        {['Python', 'Node.js', 'Electron.js', 'HTTPS', 'WebSocket', 'MFA', 'Secure Coding'].map((tech, idx) => (
                          <span
                            key={idx}
                  ...
                  >
                    ...
                    <div className={`p-6 rounded-xl ${
                      isDark ? 'bg-slate-800/50 border border-purple-500/20' : 'bg-white border border-gray-200'
                    }`}>
              ...
              >
                <div className="max-w-4xl mx-auto text-center">
                  ...
                  <motion.p
                    {...fadeInUp}
                    transition={{ delay: 0.1 }}
                    className={`text-xl ${isDark ? 'text-gray-300' : 'text-gray-700'} mb-12 max-w-2xl mx-auto`}
                  >
                    Interested in collaborating on security projects, ML research, or building innovative solutions? 
                  ...
                  >
                    ...
                    >
                      ...
                      <p className={`${isDark ? 'text-gray-400' : 'text-gray-600'}`}>atharvakanawade@gmail.com</p>
                    </motion.a>
                    ...
                    >
                      ...
                      <p className={`${isDark ? 'text-gray-400' : 'text-gray-600'}`}>Connect with me</p>
                    </motion.a>
                    ...
                    >
                      <Github className="text-cyan-400 mx-auto mb-4 group-hover:scale-110 transition-transform" size={32} />
                      <h3 className={`text-xl font-bold mb-2 ${isDark ? 'text-white' : 'text-gray-900'}`}>GitHub</h3>
                      <p className={`${isDark ? 'text-gray-400' : 'text-gray-600'}`}>Check out my code</p>
                    </motion.a>
                    ...
                    >
                      <Terminal className="text-cyan-400 mx-auto mb-4" size={32} />
                      <h3 className={`text-xl font-bold mb-2 ${isDark ? 'text-white' : 'text-gray-900'}`}>Location</h3>
                      <p className={`${isDark ? 'text-gray-400' : 'text-gray-600'}`}>Pune, India</p>
                      <p className={`text-sm ${isDark ? 'text-gray-500' : 'text-gray-500'} mt-1`}>+91 975385785</p>
                    </motion.div>
                  </motion.div>
                  ...
                  >
                    ...
                    >
                      ...
                      <ChevronDown size={18} className={`transform transition-transform ${showResumeMenu ? 'rotate-180' : ''}`} />
                    </button>
                  </motion.div>
                </div>
              </motion.section>
        ...
        {/* Footer */}
        <footer className={`relative z-10 py-8 ${isDark ? 'bg-slate-900/50 border-t border-cyan-500/20' : 'bg-white/50 border-t border-gray-200'} backdrop-blur`}>
